...
def classify(row):
    ...
    # OCM Edge cases
    if number == 'CS1674788':
        return "Integration & Interfaces", "Integration Failure", "High", "Process communication queue failure following server restart.", "No"
    if number == 'CS1715676':
        return "Configuration & Administration", "System Administration", "High", "Keystone background housekeeping process hanging.", "No"
    if number == 'CS1719997':
        return "Configuration & Administration", "System Administration", "High", "Shared folder path configuration issue for manual inspection.", "No"
    if number == 'CS1758700':
        return "Integration & Interfaces", "Third-Party Systems", "High", "ECDS / CDS sender ID configuration for national reporting feed.", "No"
    if number == 'CS1780787':
        return "Access & Security", "Authentication", "High", "SSL / TLS security certificate expiration for Keystone application.", "No"
    if number == 'CS1794144':
        return "Integration & Interfaces", "Integration Failure", "High", "GP surgery lab result outbound messaging dispatch failure.", "No"
    ...
    # Authentication
    if any(k in text for k in ['login', 'log in', 'logon', 'password', 'sso', 'mfa', 'locked out', 'lockout', 'sign in', 'signin', 'authentication', 'credential', 'can\'t log on', 'cannot log on', 'cant log on', 'certificate']):
        return "Access & Security", "Authentication", "High", "Case relates to user authentication, password reset, SSO, or certificate management.", "No"
    # Security Concern
    ...
    # Integration Failure
    if any(k in text for k in ['hl7', 'interface', 'dad', 'messaging', 'inbound', 'outbound', 'api failure', 'feed failure', 'sync failure', 'integration error', 'adt', 'orm', 'oru', 'msh', 'messages not passing', 'not going to maxims', 'messages failing', 'ecds failure', 'ecds submission', 'did not go to ipms', 'pas number', 'order messaging', 'order communications', 'lab interface', 'pathology interface', 'not receiving results', 'result dispatch']):
        return "Integration & Interfaces", "Integration Failure", "High", "Case relates to HL7 messaging, lab/pathology interop feeds, or order communications interface failures.", "No"
    # Third-Party Systems
    if any(k in text for k in ['trud', 'ods', 'dmd', 'spine', 'scr', 'tie', 'pds', 'third party', 'external system', 'vendor dependency', 'gp import', 'ice ', 'sunquest', 'telepath', 'winpath', 'cyberlab', 'cds sender']):
        return "Integration & Interfaces", "Third-Party Systems", "High", "Case involves external NHS services (SCR/TIE/TRUD/ODS) or third-party lab/vendor systems.", "No"
    # Data Exchange
    ...
    # Master Data
    if any(k in text for k in ['location', 'department', 'ward', 'clinic', 'lookup', 'master file', 'masterfile', 'reference data', 'user setup', 'team setup', 'catalogue', 'order set', 'test code']):
        return "Configuration & Administration", "Master Data", "High", "Case involves master reference data, order set configuration, test catalogues, or ward lookups.", "No"
    # Configuration Change
    if any(k in text for k in ['printing', 'print', 'printer', 'label', 'batch print', 'document output', 'pdf', 'template', 'form config', 'field config', 'wristband', 'scanner', 'midas', 'phlebotomy label', 'specimen label']):
        return "Configuration & Administration", "Configuration Change", "High", "Case involves specimen/label printing, printer configuration, forms, or templates.", "No"
    # System Administration
    if any(k in text for k in ['scheduled job', 'system setting', 'admin maintenance', 'environment', 'server config', 'low disk space', 'server in use', 'failover', 'db01', 'framework', 'housekeeping']):
        return "Configuration & Administration", "System Administration", "High", "Case involves server administration, background housekeeping, disk space, or system properties.", "No"

    # 4. Data Management
    # Data Recovery
    if any(k in text for k in ['delete', 'deleted', 'soft delete', 'restore', 'recover', 'merge', 'duplicate record', 'missing e-docs']):
        return "Data Management", "Data Recovery", "High", "Case involves record restoration, deleted data recovery, or record merging.", "No"
    # Data Correction
    if any(k in text for k in ['sql', 'database', 'corrupt', 'data fix', 'script', 'amend record', 'data correction', 'clean-up', 'cleanup', 'incorrect value', 'incorrect date', 'wrong patient']):
        return "Data Management", "Data Correction", "High", "Case involves database scripts, data corrections, patient record unlinking, or manual data fixes.", "No"
    # Data Quality
    ...
    # 6. Workflow & Process
    if any(k in text for k in ['workflow', 'stuck task', 'approval', 'routing', 'queue', 'assignment rule', 'process automation', 'checkout zone', 'order status', 'phlebotomy', 'sample collection']):
        if 'approval' in text:
            ...
            return "Workflow & Process", "Workflow Issue", "High", "Case involves automated order workflow processing, sample collection routing, or stuck tasks.", "No"

    # 11. User Guidance & Training
    if any(k in text for k in ['how to', 'how do i', 'guidance', 'help with', 'training', 'knowledge article', 'user guide']):
        return "User Guidance & Training", "How-To Request", "High", "Case involves operational guidance, order entry assistance, or usage enquiries.", "No"

    # 10. Service Requests
    if any(k in text for k in ['request', 'new ', 'create', 'setup', 'add ', 'enable', 'change', 'update', 'quote']):
        return "Service Requests", "New Request", "Medium", "Case represents a service request for creation, test setup, or system updates.", "No"
...
def run_all():
    ...
    print(f"Processed {len(cases)} OCM cases.")
