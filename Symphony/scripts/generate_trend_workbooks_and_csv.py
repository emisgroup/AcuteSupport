...
def main():
    ...
    logging.info("Reading raw Symphony cases and SLA CSV files...")
    audit_df = pd.read_csv(raw_dir / "Sym-Audit_Cases_Last_12-Months.csv", encoding="cp1252")
    dad_df = pd.read_csv(raw_dir / "Sym-DAD_Cases_Last_12-Months.csv", encoding="cp1252")
    sla_df = pd.read_csv(raw_dir / "Sym_SLA_Last_12-Months.csv", encoding="utf-8-sig")

    audit_df.insert(0, "Trend", "Audit")
    dad_df.insert(0, "Trend", "DAD")

    all_cases = pd.concat([audit_df, dad_df], ignore_index=True)
    logging.info(f"Loaded {len(all_cases)} total cases across 2 trends (Audit: {len(audit_df)}, DAD: {len(dad_df)}).")
    ...
    # 1. Save Combined CSV
    combined_csv_path = out_dir / "Symphony_Combined_Cases_With_SLA.csv"
    proc_csv_path = proc_dir / "Symphony_Combined_Cases_With_SLA.csv"
    ...
    master_wb_path = out_dir / "Symphony_Cases_Trend_Workbooks_With_SLA.xlsx"
    with pd.ExcelWriter(master_wb_path, engine="openpyxl") as writer:
        for sheet_name in ["Combined", "Audit", "DAD"]:
    ...
    for trend in ["Audit", "DAD"]:
        trend_df = final_df[final_df["Trend"] == trend]
        
        # Individual CSV
        trend_csv_path = out_dir / f"Symphony_{trend}_Cases_With_SLA.csv"
        ...
        trend_xlsx_path = out_dir / f"Symphony_{trend}_Cases_With_SLA.xlsx"
    ...
    print(f"2. Master Excel Workbook (Tabs: Combined, Audit, DAD): {master_wb_path}")
    print(f"3. Individual Trend Workbooks:")
    print(f"   - Audit: {out_dir / 'Symphony_Audit_Cases_With_SLA.xlsx'}")
    print(f"   - DAD: {out_dir / 'Symphony_DAD_Cases_With_SLA.xlsx'}")
    print(f"4. Individual Trend CSVs:")
    print(f"   - Audit: {out_dir / 'Symphony_Audit_Cases_With_SLA.csv'}")
    print(f"   - DAD: {out_dir / 'Symphony_DAD_Cases_With_SLA.csv'}")
